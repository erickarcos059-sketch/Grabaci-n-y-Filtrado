...
int16_t aplicar_filtro_anti_aire(int16_t muestra) {
    ...
    lpf_y = 0.21f * x + 0.79f * lpf_y;                     // LPF one-pole ~150 Hz (antes 0.10f/0.90f = 67 Hz)
    float hpf_y = lpf_y - hpf_x_prev + 0.96f * hpf_y_prev; // HPF one-pole ~26 Hz  (antes 0.92f = 53 Hz)
    hpf_x_prev = lpf_y;
    hpf_y_prev = hpf_y;
